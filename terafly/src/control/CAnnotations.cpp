...
#ifdef terafly_enable_debug_annotations
        tf::debug(tf::LEV_MAX, strprintf("Added neuron %lld(%lld) {%.0f, %.0f, %.0f} to annotations list of octant X[%d,%d] Y[%d,%d] Z[%d,%d]", neuron.ID, neuron.parent ? neuron.parent->ID : -1, neuron.x, neuron.y, neuron.z, p_octant->H_start, p_octant->H_start + p_octant->H_dim, p_octant->V_start, p_octant->V_start + p_octant->V_dim, p_octant->D_start, p_octant->D_start + p_octant->D_dim).c_str(), 0, true);
#endif
    }
}
...
#ifdef terafly_enable_debug_annotations
        tf::debug(tf::LEV_MAX, strprintf("REMOVED neuron %lld(%lld) {%.0f, %.0f, %.0f} from annotations list of octant X[%d,%d] Y[%d,%d] Z[%d,%d]", neuron->ID, neuron->parent ? neuron->parent->ID : -1, neuron->x, neuron->y, neuron->z, p_octant->H_start, p_octant->H_start + p_octant->H_dim, p_octant->V_start, p_octant->V_start + p_octant->V_dim, p_octant->D_start, p_octant->D_start + p_octant->D_dim).c_str(), 0, true);
#endif
    }
}
...
void CAnnotations::Octree::find(interval_t V_int, interval_t H_int, interval_t D_int, std::list<annotation *> &neurons) throw(RuntimeException)
{
    ...
    if (H_int.start < 0 || H_int.end < 0 || (H_int.end - H_int.start < 0) ||
        ...
        throw tf::RuntimeException(tf::strprintf("invalid interval X[%d,%d), Y[%d,%d), Z[%d,%d)",
                                                 H_int.start, H_int.end, V_int.start, V_int.end, D_int.start, D_int.end),
                                   tf::shortFuncName(__itm__current__function__));
...
{
    /**/ tf::debug(tf::LEV1, strprintf("X[%d,%d), Y[%d,%d), Z[%d,%d), markers.size = %d", X_range.start, X_range.end, Y_range.start, Y_range.end, Z_range.start, Z_range.end, markers.size()).c_str(), __itm__current__function__);
...
void CAnnotations::addCurves(tf::interval_t X_range, tf::interval_t Y_range, tf::interval_t Z_range, NeuronTree &nt) throw(tf::RuntimeException)
{
    ...
    for (std::map<int, annotation *>::iterator it = annotationsMap.begin(); it != annotationsMap.end(); it++)
    {
        ...
        {
#ifdef terafly_enable_debug_annotations
            tf::debug(tf::LEV_MAX, strprintf("Add %lld(%.0f, %.0f, %.0f) to %lld(%.0f, %.0f, %.0f)'s children list\n", it->second->ID, it->second->x, it->second->y, it->second->z, it->second->parent->ID, it->second->parent->x, it->second->parent->y, it->second->parent->z).c_str(), 0, true);
#endif
...
{
    /**/ tf::debug(tf::LEV1, strprintf("X_range = [%d,%d), Y_range = [%d,%d), Z_range = [%d,%d)", X_range.start, X_range.end, Y_range.start, Y_range.end, Z_range.start, Z_range.end).c_str(), __itm__current__function__);
    ...
    else
    {
        ...
        for (V3DLONG tip = 0; tip < tipnodes.size(); tip++)
        {
            for (V3DLONG r = 0; r < rootnodes.size(); r++)
            {
                ...
                if (neuron.at(tipnodes.at(tip)).x == neuron.at(rootnodes.at(r)).x && neuron.at(tipnodes.at(tip)).y == neuron.at(rootnodes.at(r)).y && neuron.at(tipnodes.at(tip)).z == neuron.at(rootnodes.at(r)).z)
                {
...
QHash<V3DLONG, V3DLONG> getUniqueLUT(QList<NeuronSWC> &neurons)
{
    ...
    for (V3DLONG i = 0; i < neurons.size(); i++)
    {
        ...
        {
            if (neurons.at(i).x == neurons.at(j).x && neurons.at(i).y == neurons.at(j).y && neurons.at(i).z == neurons.at(j).z)
                break;
        }
        if (i == j)
        { // not a duplicate of the previous ones
...
bool CAnnotations::Sort_SWC_NewVersion(QList<NeuronSWC> &neurons, QList<NeuronSWC> &result, V3DLONG newrootid)
{
    ...
    if ((thres != 1000000000) && (thres > 0))
    { // If distance threshold > 1: make new connections
        ...
        while (cur_group > 1)
        {
            ...
            if (min <= thres * thres)
            {
                ...
                if (!neighbors.at(m1).contains(m2))
                {
                    neighbors[m1].push_back(m2);
                }
                if (!neighbors.at(m2).contains(m1))
                {
                    neighbors[m2].push_back(m1);
                }
            }
            else
            { // set component the node closest to group 1 is root
...
void CAnnotations::save(const char *filepath, bool removedupnode, bool as_swc) throw(RuntimeException)
{
    ...
    cout << endl
         << "output_ano: " << qPrintable(fileprefix + output_ano) << endl;
    ...
    if (removedupnode)
    {
        ...
        if (soma_found_new == 0)
        { // Case 1: If soma_found_new == 0, try using the old soma location
        ...
        if (SOMA_FOUND > 1)
        { // Case 2: If soma_found_new > 0, use the new soma location
    ...
    for (std::string line; std::getline(f, line);)
    {
        ...
        else if (tokens[0].compare("SWCFILE") == 0)
        {
            ...
            for (std::map<int, annotation *>::iterator i = annotationsMap.begin(); i != annotationsMap.end(); i++)
            {
                ...
                {
#ifdef terafly_enable_debug_annotations
                    tf::debug(tf::LEV_MAX, strprintf("Add %lld(%.0f, %.0f, %.0f) to %lld(%.0f, %.0f, %.0f)'s children list\n", i->second->ID, i->second->x, i->second->y, i->second->z, i->second->parent->ID, i->second->parent->x, i->second->parent->y, i->second->parent->z).c_str(), 0, true);
#endif
...
tf::CAnnotations::typeIandIIerrorAPO(std::string apo1Path, // first apo file path (assumed as TRUTH)
                                     ...
                                     const std::string &outputPath /*=*/) throw(tf::RuntimeException)
{
    // read cells
...
{
    /**/ tf::debug(tf::LEV1, strprintf("apo1Path = \"%s\", apo2Path = \"%s\", x0 = %d, x1 =%d, y0 = %d, y1 = %d, z0 = %d, z1 = %d", apo1Path.c_str(), apo2Path.c_str(), x0, x1, y0, y1, z0, z1).c_str(), __itm__current__function__);
