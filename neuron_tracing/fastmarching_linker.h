...
template <class T>
bool fastmarching_linker(vector<MyMarker> &sub_markers, vector<MyMarker> &tar_markers,
						 T *inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2) // float time,
{
	...
	if (tol_sz <= 0)
	{
		cout << "wrong size info in fastmarching_linker" << endl;
		return false;
	}
	...
	max_int -= min_int;
	if (max_int == 0.0)
		return false; // no image data, avoid divide by zero in GI
	double li = 10;
	...
	V3DLONG *parent = new V3DLONG[tol_sz];
	...
	if (!elems.empty())
		for (map<V3DLONG, HeapElemX *>::iterator it = elems.begin(); it != elems.end(); it++)
			delete it->second;
	if (phi)
	{
		delete[] phi;
...
template <class T>
bool fastmarching_linker_timer(vector<MyMarker> &sub_markers, vector<MyMarker> &tar_markers,
							   T *inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2) // float time,
{
	int ALIVE = -1;
	int TRIAL = 0;
...
template <class T>
bool fastmarching_linker(MyMarker nm1, MyMarker fm1, MyMarker nm2, MyMarker fm2, T *inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2)
{
...
#define MAKE_UNIT(V)                                                \
	...
	unsigned char *outimg1d = new unsigned char[btol_sz];
	for (long i = 0; i < btol_sz; i++)
		outimg1d[i] = 0;
...
template <class T>
bool fastmarching_drawing1(vector<MyMarker> &near_markers, vector<MyMarker> &far_markers, T *inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2)
{
...
template <class T>
bool fastmarching_drawing2(vector<MyMarker> &near_markers, vector<MyMarker> &far_markers, T *inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2)
{
	assert(near_markers.size() == far_markers.size() && near_markers.size() >= 2);
 ...
 * *****************************************************************************/
template <class T>
bool fastmarching_linker(map<MyMarker *, double> &sub_markers, map<MyMarker *, double> &tar_markers, T *inimg1d, vector<MyMarker *> &par_tree, int sz0, int sz1, int sz2, int stop_num, int cnn_type = 2)
{
...
//
template <class T>
bool fastmarching_linker(map<MyMarker *, double> &sub_markers, map<MyMarker *, double> &tar_markers, T *inimg1d, vector<MyMarker *> &par_tree, int sz0, int sz1, int sz2, MyMarker nm1, MyMarker fm1, MyMarker nm2, MyMarker fm2, int stop_num, int cnn_type = 2, int margin = 5, double *intensityThreshold = 0)
{
		...
		for (long k = 0; k < bsz2; k++)
		{
			for (long j = 0; j < bsz1; j++)
			{
				for (long i = 0; i < bsz0; i++)
				{
					...
					{
						long ind2 = kk * sz01 + jj * sz0 + ii;
						if (inimg1d[ind2] > max_int)
							max_int = inimg1d[ind2];
						if (inimg1d[ind2] < min_int)
							min_int = inimg1d[ind2];
					}
				}
			}
		}
...
template <class T>
bool fastmarching_drawing_dynamic(vector<MyMarker> &near_markers, vector<MyMarker> &far_markers, T *inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2, int margin = 5, bool intensityThresholdmode = false)
{
	...
	for (map<MyMarker *, double>::iterator it = tar_markers.begin(); it != tar_markers.end(); it++)
		all_markers.push_back(it->first);
	...
	for (int i = 1; i < near_markers.size(); i++)
	{
		...
		near_marker1 = near_marker2;
		far_marker1 = far_marker2;
		near_marker2 = near_markers[i];
		far_marker2 = far_markers[i];

		sub_markers.clear();
		sub_markers = tar_markers;
		tar_markers.clear();
		GET_LINE_MARKER_MAP(near_marker2, far_marker2, tar_markers);
	...
	MyMarker *new_marker = new MyMarker(p->x, p->y, p->z);
	outswc.push_back(new_marker);
	MyMarker *child_marker = new_marker;
	p = p->parent;
	while (p)
	{
		new_marker = new MyMarker(p->x, p->y, p->z);
		outswc.push_back(new_marker);
		child_marker->parent = new_marker;
		child_marker = new_marker;
		if (p->parent == 0)
...
template <class T>
bool fastmarching_drawing_serialbboxes(vector<MyMarker> &near_markers, vector<MyMarker> &far_markers, T *inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2, int margin = 5, bool intensityThresholdmode = false)
{
	if (intensityThresholdmode)
	...
	for (V3DLONG m = 1; m < near_markers.size(); m++)
	{
		...
		if (nm1 != fm1)
		{
			...
			double tz1 = fm1.z - nm1.z;
			double dst1 = sqrt(tx1 * tx1 + ty1 * ty1 + tz1 * tz1);
			qDebug() << "dst1 = " << dst1 << "tx1 = " << tx1 << "ty1 = " << ty1 << "tz1 = " << tz1;
			rt[0] = tx1 / dst1;
			rt[1] = ty1 / dst1;
			rt[2] = tz1 / dst1;
		}
	...
	unsigned char *mskimg1d = new unsigned char[mtol_sz];
	memset(mskimg1d, 0, mtol_sz);
	double max_int = 0; // maximum intensity
	double min_int = INF;
	double intensityThreshold = 0;
	if (intensityThresholdmode)
		...
		for (V3DLONG z = 0; z < msz2; z++)
		{
			for (V3DLONG y = 0; y < msz1; y++)
			{
				for (V3DLONG x = 0; x < msz0; x++)
				{
					...
					if (inimg1d[marker.ind(sz0, sz01)] > max_int)
						max_int = inimg1d[marker.ind(sz0, sz01)];
					if (inimg1d[marker.ind(sz0, sz01)] < min_int)
						min_int = inimg1d[marker.ind(sz0, sz01)];
	...
	nm1 = near_markers[0];
	fm1 = far_markers[0];
	nm2 = *near_markers.rbegin();
	fm2 = *far_markers.rbegin();
...
template <class T>
bool fastmarching_drawing5(vector<MyMarker> &near_markers, vector<MyMarker> &far_markers, T *inimg1d, vector<MyMarker *> &outswc, int sz0, int sz1, int sz2, int cnn_type = 2)
{
	assert(near_markers.size() == far_markers.size());
	...
	unsigned char *outimg1d = new unsigned char[mtol_sz];
	memset(outimg1d, 0, mtol_sz);
